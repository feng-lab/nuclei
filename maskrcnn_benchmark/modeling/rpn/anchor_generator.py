...
class AnchorGenerator(nn.Module):
    ...
    def forward(self, image_list, feature_maps):
        ...
        # print('gi', grid_sizes, image_list.image_sizes)
        anchors = []
...
def _generate_anchors(base_size, scales, aspect_ratios):
    ...
    """
    anchor = np.array([1, 1, base_size + 1, base_size + 1], dtype=np.float) - 1
...
def _whctrs(anchor):
    ...
    w = anchor[2] - anchor[0] + 1 - 1
    h = anchor[3] - anchor[1] + 1 - 1
    x_ctr = anchor[0] + 0.5 * (w - 1 + 1)
    y_ctr = anchor[1] + 0.5 * (h - 1 + 1)
