...
def prepare_for_coco_segmentation(predictions, dataset):
    ...
    for image_id, prediction in tqdm(enumerate(predictions)):
        ...
        # masks = [scipy.ndimage.binary_dilation(mask[0, :, :] > 0, iterations=1) for mask in masks]
        masks = [mask[np.newaxis, :, :].astype(np.uint8) for mask in masks]
