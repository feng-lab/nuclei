...
class BinaryMaskList(object):
    ...
    def crop(self, box):
        ...
        assert xmin < xmax and ymin < ymax, (str(box), xmin, xmax, ymin, ymax)
        ...
        for mask in masks:
            if use_cv2:
                ...
                contour, hierarchy = cv2_util.findContours(
                    mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE  # cv2.CHAIN_APPROX_TC89_L1
                )
                ...
                contour = skimage.measure.find_contours(mask.astype('uint8'), level=0.5)
                reshaped_contour = []
                for entity in contour:
...
class PolygonInstance(object):
    ...
    def convert_to_binarymask(self):
        # assert False
        width, height = self.size
        is_poly_integer = all(torch.all(torch.round(p) == p) for p in self.polygons)
        if is_poly_integer:
            mask = np.zeros((height, width), dtype=np.uint8)
            mask = PIL.Image.fromarray(mask)
            polygons = [p.numpy() for p in self.polygons]
            for p in polygons:
                PIL.ImageDraw.Draw(mask).polygon(xy=p, outline=1, fill=1)
            mask = np.array(mask)
            assert mask.dtype == np.uint8, mask
