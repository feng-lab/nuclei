...
def _rename_weights_for_resnet(weights, stage_names):
    ...
    for k in original_keys:
        ...
        # if 'fc1000' in k:
        #     continue
        logger.info("C2 name: {: <{}}".format(k, max_c2_key_size))
        # logger.info(v)
        if 'weight_order' == k:
            continue
        if 'bbox_pred_w' == k or 'bbox_pred_b' == k:
            continue
        if 'cls_score_w' == k or 'cls_score_b' == k:
            continue
        if 'mask_fcn_logits_w' == k or 'mask_fcn_logits_b' == k:
            continue
